...
Game.GameName = "Call of Duty Black Ops Multiplayer";
...
Game.Hook.ForceFocusWindowName = "Call of Duty®: Black Ops";
Game.SupportsKeyboard = true;
Game.KillProcessesOnClose = ["END", "cmd", "Server"];
Game.SteamID = "42700";
Game.GUID = "CallofDutyBlackOpsMP";
...
Game.Play = function() {
  ...
  if (!System.IO.File.Exists(Context.GetFolder(Nucleus.Folder.InstancedGameFolder) + "\\BlackOpsMP - Player " + (Context.PlayerID + 1) + ".exe")) {
    var savePath = Context.SavePath = Context.GetFolder(Nucleus.Folder.InstancedGameFolder) + "\\BlackOpsMP - Player " + (Context.PlayerID + 1) + ".exe"; 
    var savePkgOrigin = System.IO.Path.Combine(Game.Folder, "BGT5Package\\BlackOpsMP.exe"); 
    System.IO.File.Copy(savePkgOrigin, savePath, true);
  }
  ...
  var txtPath = Context.GetFolder(Nucleus.Folder.InstancedGameFolder) + "\\players\\config_mp.cfg";
  Context.ReplaceLinesInTextFile(
    txtPath,
    [
